...
module.on("componentpage", () => {
	const reactionsButton = document.querySelector('.js-reactions_button');
	...
	const updateFloatingButton = () => {
		...
		if (newReactionsCnt > 0) {
			if (!cleanupFloatingButton) {
				const topAnchor = document.querySelector('.js-unread_reactions_top_anchor');
				const bottomAnchor = document.querySelector('.js-unread_reactions_bottom_anchor');
				reactionsButton.classList.remove('unread-reactions-button--is-hidden');
				cleanupFloatingButton = useFloatingButton(reactionsButton, {
					topAnchor,
					bottomAnchor,
					stickyClass: 'unread-reactions-button--is-sticky',
					stickyMargin: 14,
				});
			}
		} else {
	...
	pushstream.on("message", "reactions_button", (message) => {
		switch (message.act) {
...
module.on("componentpagedone", () => {
	pushstream.off("message", "reactions_button");
...
function useFloatingButton(reactionsButton, { topAnchor, bottomAnchor, stickyClass, stickyMargin }) {
	...
	const handleScroll = throttleRaf(() => {
		...
		if (position == "sticky") {
			...
			reactionsButton.style.setProperty('--unread-reactions-button-y', `${Math.round(offset)}px`);
