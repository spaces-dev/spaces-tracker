...
(function (window, document, JS_URL, CSS_URL, MAIN_REVISION) {
	...
	*/
	...
	function requireAsync(names, callback, errback, context) {
		...
		let builtin = {
			...
			loader:		{executeScripts, loadScript, tick, clearTick, domReady, windowReady}
		};
	...
	*/
	...
	function loadModuleAsset(name) {
		...
		let asset_url = getModuleUrl(name, true);
		if (name.indexOf('.css') > 0) {
			loadStyle(asset_url, module_to_sort[name]);
		} else {
