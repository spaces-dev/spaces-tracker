...
var ITEM_WIDTH = Device.touch_light ? 32 : 21;
var TOOLBAR_ITEMS = {
...
var tpl = {
	toolbar: function (data) {
		var html = '<table class="bb_table' + (data.hide ? ' hide' : '') + '" id="bbtable_' + data.id + '"><tr>';
		each(TOOLBAR_ITEMS, function (d, tag) {
			if ((data.disable.bgcolor && tag == 'fon'))
				return;
			if (data.disable[tag])
				return;
		...
		html += '</tr><tr class="hide"></tr><tr class="hide"></tr><tr class="hide"></tr></table>';
...
var Toolbar = function (el, options) {
	...
	function setupToolbar() {
		...
		var active_attaches = document.getElementsByName('active_attaches')[0];
		if (active_attaches) {
		...
		if (options.toolbar) {
			addEvent(window, 'resize', fixToolbarWidth);
			addEvent(window, 'orientationchange', fixToolbarWidth);
			tick(fixToolbarWidth);
			setTimeout(fixToolbarWidth, 1000);
		}

		var inputs = document.getElementsByTagName('button');
		for (var i = 0; i < inputs.length; ++i) {
			var input = inputs[i];
			if (input.name == 'smiles_btn') {
				input.onclick = function () {
					ge('.bb_smiles', toolbar)[0].click();
					return false;
				};
			} else if (hasClass(input, 'js-bb_toggle')) {
				input.onclick = function () {
					toggleToolbar(options.hide);
					return false;
				};
			}
		}
	...
	function insertTag() {
		...
		if (tag == 'smiles') {
			const smile_btn = document.getElementsByName("smiles_btn")[0];
			const smile_btn_img = smile_btn?.getElementsByTagName('img')[0];
			...
			if (smile_btn_img)
				smile_btn_img.src = ICO_SPINNER;

			import("./smiles_menu").then(function ({SmilesMenu}) {
				SmilesMenu.toggle(id, toolbar, function (value) {
					textReplaceTag(el, function (v) {
						return value + ' ';
					});
				}, () => {
					ico.src = `${ICONS_BASEURL}ico/smile_black.png`;
					if (smile_btn_img)
						smile_btn_img.src = `${ICONS_BASEURL}ico/smile_black.png`;
				}, () => {
					ico.src = `${ICONS_BASEURL}ico/smile.png`;
					if (smile_btn_img)
						smile_btn_img.src = `${ICONS_BASEURL}ico/smile.png`;
				}, {stickers: !options.disable.stickers});
			});
		} else if ((tag == 'color' || tag == 'fon') && !val) {
