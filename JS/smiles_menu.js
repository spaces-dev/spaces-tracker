...
import {L, ge, ce, hasClass, removeClass, addClass, tick} from './utils';
import $ from './jquery';

import "CommentWidget/SmilesMenu.css";
...
const tpl = {
	menuHeader() {
		const tabs = [
			...
			{
				title: `<span class="ico ico_remove ico-inline"></span>`,
				type: "close"
			}
		];
		return `
				...
				${categories.map((cat) => {
					return `
						<a href="#" class="smiles-menu__category js-smile_menu_nav" data-type="smiles" data-page="list" data-id="${cat.id}">
							...
							<img
								class="smiles-menu__category-icon"
								src="${cat.icon.src}"
	...
	smilesList(id) {
		const categoryName = db.smiles[id].name;
		let maxWidth = 0;
		let maxHeight = 0;
...
function initMenu(id, toolbar) {
	...
	$(menu).on("click", ".js-smile_menu_nav", (e) => {
		e.preventDefault();
		const link = $(e.currentTarget);

		if (link.data('type') == "close") {
			closeMenu();
			return;
		}
		...
		render();
	}).on("click", ".js-smile_menu_select", (e) => {
		e.preventDefault();
		const link = $(e.currentTarget);
		const callback = insertTagCallback;
		closeMenu();
		callback(link.data('value'));
	}).on("click", ".js-smile_menu_close", (e) => {
		e.preventDefault();
		closeMenu();
	});
}
