...
jQuery.extend = jQuery.fn.extend = function() {
	if (arguments.length == 1) {
		var target = this, source = arguments[0];
		for (var k in source) {
			if (hasOwn.call(source, k))
				target[k] = source[k];
		}
		return this;
	}
	...
	jQuery.extend({
			...
			if ( seed ) {
				...
				while (selector.length && (m = rquickselectors.exec(selector))) {
					...
					// tag name
						...
						let collection = context.getElementsByTagName(m[2])
						if (collection.length) {
							push.apply(results, collection);
							mixed++;
						}
					}
					// .class
					else if (m[3]) {
						let collection = context.getElementsByClassName(m[3])
						if (collection.length) {
							push.apply(results, collection);
							mixed++;
						}
		...
		expr: {
			...
			match: {
				...
				needsContext: new RegExp( "^" + whitespace + "*[>+~]|:(scope)(?:\\(" + whitespace + "*((?:-\\d)?\\d*)" + whitespace + "*\\)|)(?=[^-]|$)", "i" )
			}
		}
	});
...
function internalData( elem, name, data, pvt /* Internal Use Only */ ) {
	...
	if ( data !== undefined ) {
		...
		if (typeof data != "object" && elem.dataset != null) {
			if (data != null) {
				elem.dataset[jQuery.camelCase(name)] = data ?? '';
			} else {
				delete elem.dataset;
			}
		}
	...
	if ( name ) {
		...
		if ( thisCache ) {
			...
			while ( i-- ) {
				...
				if (elem && elem.dataset != null)
					delete elem.dataset[jQuery.camelCase(name[i])];
			}
...
function adjustCSS( elem, prop, valueParts, tween ) {
	var adjusted,
		scale = 1,
		maxIterations = 20,
		currentValue = function() { return jQuery.css( elem, prop, "" ); },
	...
	add: function( elem, types, handler, data, selector ) {
		...
		if ( !( eventHandle = elemData.handle ) ) {
			eventHandle = elemData.handle = function( e ) {
				var type;
				try { type = e.type } catch (e) { /* падает с FireFox */ }
				// Discard the second event of a jQuery.event.trigger() and
				// when an event is called after a page has unloaded
				return typeof jQuery !== "undefined" &&
					( !e || jQuery.event.triggered !== type ) ?
...
support.reliableHiddenOffsets = returnTrue;
support.boxSizingReliable = returnTrue;
support.pixelMarginRight = returnTrue;
support.pixelPosition = returnTrue;
support.reliableMarginRight = returnTrue;
support.reliableMarginLeft = returnTrue;
